...
export function AppSidebar({ profile }: AppSidebarProps) {
  ...
  return (
    <aside
      className={cn(
        'hidden lg:flex flex-col justify-between border-r border-border bg-card/40 transition-all duration-300 relative z-30 shrink-0 h-screen',
      ...
      <div>
        <div className="h-16 flex items-center justify-between px-4 border-b border-border">
          ...
          <button
            ...
            className="hidden lg:flex h-5 w-5 items-center justify-center rounded-md border border-border bg-background text-muted-foreground hover:text-foreground hover:bg-muted absolute -right-2.5 top-5.5 z-40 transition-colors shadow-sm"
          >
            {isCollapsed ? (
